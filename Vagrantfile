...
Vagrant.configure(2) do |config|
	...
	vm_config.vm.provision "shell", inline: "mkdir -p /var/lib/go-server/plugins/external"
        vm_config.vm.provision "shell", inline: "chown -R go:go /var/lib/go-server/"	
	vm_config.vm.provision "shell", inline: "mkdir -p /var/lib/go-server/plugins/external"
	vm_config.vm.provision "shell", inline: "chown -R go:go /var/lib/go-server/plugins/external"
        vm_config.vm.provision "shell", inline: "curl --silent --location --fail --retry 3 https://github.com/gocd/kubernetes-elastic-agents/releases/download/v3.0.0-156/kubernetes-elastic-agent-3.0.0-156.jar --output /var/lib/go-server/plugins/external/kubernetes-elastic-agent.jar"
